"""LEGACY (pre-2026-05-13): per-frame Shannon-entropy disagreement scoring.

Superseded by `src/data_construction/philosophy_panel/filter.py` (Stage 1
unanimity filter on the (frame_A, frame_B) tuple per philosophy) and
`src/data_construction/philosophy_panel/label.py` (Stage 2 (Y,N) vs (N,Y)
conflict labeling). This file computes within-frame entropy of the 5 modal
votes, which mixes "all phil agree per frame but disagree across frames" with
"phil disagree within a single frame" and therefore can't isolate the
PNAS-style flip-direction conflict the new pipeline targets. Kept for
reference / reproducing prior reports; do not extend.
