...
- `src/` — production source tree, split by research concern (see Directory layout below).
  ...
  - Other `src/data_construction/{inaction_labeling}/`, `src/evaluation/{runners,metrics,mitigation}/`, `src/analysis/` packages are scaffolded but mostly empty; new code lands here, not at the `src/` root.
...
Reference docs:
- `RESEARCH_PLAN_v4.md` — current research plan (EMNLP 2026 long paper; v1–v3 superseded). **Read this first** for scope, RQs, timeline. E7 mitigation detail is a SEPARATE supplement at `~/.claude/plans/research-plan-v4-giggly-nebula.md` (outside repo) — when editing E7/§5/§8/§12, keep both in sync.
